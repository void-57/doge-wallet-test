(function (EXPORTS) { //floBlockchainAPI v3.1.4
    ...
    const createTx = function (senderAddr, receiverAddr, sendAmt, strict_utxo = true) {    
        return new Promise((resolve, reject) => {
/*            if (!floCrypto.validateASCII(floData))
   ...
   //       else if (!floCrypto.validateFloID(senderAddr, true)) //Dogechange 
    ...
    floBlockchainAPI.createTx = function (senderAddr, receiverAddr, sendAmt, strict_utxo = true) {
        return new Promise((resolve, reject) => {
...
//  const sendTx = floBlockchainAPI.sendTx = function (senderAddr, receiverAddr, sendAmt, privKey, floData = '', strict_utxo = true) { //Dogechange
    const sendTx = floBlockchainAPI.sendTx = function (senderAddr, receiverAddr, sendAmt, privKey, strict_utxo = true) {
        return new Promise((resolve, reject) => {
/*            if (!floCrypto.validateFloID(senderAddr, true))
       ...
       //   createTx(senderAddr, receiverAddr, sendAmt, floData, strict_utxo).then(trx => { //Dogechange
            createTx(senderAddr, receiverAddr, sendAmt, strict_utxo).then(trx => {    
...
//  floBlockchainAPI.mergeUTXOs = function (floID, privKey, floData = '') { //Dogechange
    floBlockchainAPI.mergeUTXOs = function (floID, privKey) {        
     ...
     //     if (!floCrypto.validateFloID(floID, true)) //Dogechange
            if (!floCrypto.validateDogeID(floID, true))   //Create this new function
...
//  const sendTxMultiple = floBlockchainAPI.sendTxMultiple = function (senderPrivKeys, receivers, floData = '') { //Dogechange
    const sendTxMultiple = floBlockchainAPI.sendTxMultiple = function (senderPrivKeys, receivers) {
        return new Promise((resolve, reject) => {
/*            if (!floCrypto.validateASCII(floData))
                return reject("Invalid FLO_Data: only printable ASCII characters are allowed");*/ //Dogechange
            let senders = {},
...
//  const createMultisigTx = function (redeemScript, receivers, amounts, floData = '', strict_utxo = true) { //Dogechange
    const createMultisigTx = function (redeemScript, receivers, amounts, strict_utxo = true) {
        return new Promise((resolve, reject) => {
...
//  floBlockchainAPI.createMultisigTx = function (redeemScript, receivers, amounts, floData = '', strict_utxo = true) { //Dogechange
    floBlockchainAPI.createMultisigTx = function (redeemScript, receivers, amounts, strict_utxo = true) {    
        return new Promise((resolve, reject) => {
...
//  const sendMultisigTx = floBlockchainAPI.sendMultisigTx = function (redeemScript, privateKeys, receivers, amounts, floData = '', strict_utxo = true) { //Dogechange
    const sendMultisigTx = floBlockchainAPI.sendMultisigTx = function (redeemScript, privateKeys, receivers, amounts, strict_utxo = true) {
        return new Promise((resolve, reject) => {
            var multisig = floCrypto.decodeRedeemScript(redeemScript);
            if (!multisig)
                return reject(`Invalid redeemScript`);
       ...
       //   createMultisigTx(redeemScript, receivers, amounts, floData, strict_utxo).then(trx => { //Dogechange
            createMultisigTx(redeemScript, receivers, amounts, strict_utxo).then(trx => {    
                for (let pk of privateKeys)
                    trx.sign(pk, 1);
                var signedTxHash = trx.serialize();
                broadcastTx(signedTxHash)
    ...
    return new Promise((resolve, reject) => {
        ...
        readTxs(addr, query_options).then(response => {
            ...
            if (!isUndefined(options.after)) {
                ...
                if (i_after !== -1)  // found lastItem, remove it and all txs before that
                    response.items.splice(i_after);
            }

            // find first tx on this page that matches filters + caseFn
            const item = response.items.find(tx => {
                if (!tx.confirmations)  // unconfirmed should be filtered upstream, but double-guard
                    return false;

                // sent filters
                if (options.sentOnly && !tx.vin.some(vin => vin.addresses?.[0] === addr))
                    return false;
                if (Array.isArray(options.senders) && !tx.vin.some(vin => options.senders.includes(vin.addresses?.[0])))
                    return false;

                // received filters
                if (options.receivedOnly && !tx.vout.some(vout => vout.scriptPubKey?.addresses?.[0] === addr))
                    return false;
                if (Array.isArray(options.receivers) && !tx.vout.some(vout => options.receivers.includes(vout.scriptPubKey?.addresses?.[0])))
                    return false;
            ...
            if (!isUndefined(item)) {
                ...
                if (options.tx) {
                    result.item = {
                        ...
                        senders: new Set(item.vin.map(v => v.addresses?.[0]).filter(Boolean)),
                        receivers: new Set(item.vout.map(v => v.scriptPubKey?.addresses?.[0]).filter(Boolean)),
                    };
                } else {
