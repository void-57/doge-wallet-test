(function (EXPORTS) {
  ...
  floCrypto.generateMultiChain = function (inputWif) {
    try {
      ...
      // Generate addresses and WIFs for each chain
      // For DOGE
      ...
      if (sourceWif !== inputWif) {
        ...
        result.DOGE.wif = bitjs.privkey2wif(key.privkey);
      } else {
      ...
      if (sourceWif !== inputWif) {
        const key = bitjs.wif2privkey(sourceWif);
        result.BTC.wif = bitjs.privkey2wif(key.privkey);
      } else {
        const key = bitjs.wif2privkey(inputWif);
        result.BTC.wif = bitjs.privkey2wif(key.privkey);
      }
      ...
      if (sourceWif !== inputWif) {
        const key = bitjs.wif2privkey(sourceWif);
        result.FLO.wif = bitjs.privkey2wif(key.privkey);
      } else {
        const key = bitjs.wif2privkey(inputWif);
        result.FLO.wif = bitjs.privkey2wif(key.privkey);
      }
