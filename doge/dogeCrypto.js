(function (EXPORTS) {
  ...
  //  floCrypto.validateFloID = function (floID, regularOnly = false) { //Dogechange
  floCrypto.validateDogeID = function (floID, regularOnly = false) {
  ...
  //Generates multi-chain addresses (DOGE, BTC, FLO) from the given WIF or new WIF
  floCrypto.generateMultiChain = function (inputWif) {
    try {
      ...
      let sourceWif = inputWif;
      let pubKey;

      if (typeof inputWif === "string" && inputWif.length > 0) {
        // Use provided WIF 
        const pubKeyObj = bitjs.wif2pubkey(inputWif);
        pubKey = pubKeyObj.pubkey;
      } else {
        // Generate a new key
        const newKey = generateNewID();
        sourceWif = newKey.privKey;
      ...
      // Generate addresses and private keys for each chain
      // For DOGE
      bitjs.pub = versions.DOGE.pub;
      bitjs.priv = versions.DOGE.priv;
      result.DOGE.address = bitjs.pubkey2address(pubKey);
      if (sourceWif !== inputWif) {
        // If we generated a new key, create a new WIF with DOGE prefix
        const key = bitjs.wif2privkey(sourceWif);
        result.DOGE.privateKey = bitjs.privkey2wif(key.privkey);
      } else {
        // If input was already a WIF, just convert it
        const key = bitjs.wif2privkey(inputWif);
        result.DOGE.privateKey = bitjs.privkey2wif(key.privkey);
      }
      ...
      if (sourceWif !== inputWif) {
        const key = bitjs.wif2privkey(sourceWif);
        result.BTC.privateKey = bitjs.privkey2wif(key.privkey);
      } else {
        const key = bitjs.wif2privkey(inputWif);
        result.BTC.privateKey = bitjs.privkey2wif(key.privkey);
      }
      ...
      if (sourceWif !== inputWif) {
        const key = bitjs.wif2privkey(sourceWif);
        result.FLO.privateKey = bitjs.privkey2wif(key.privkey);
      } else {
        const key = bitjs.wif2privkey(inputWif);
        result.FLO.privateKey = bitjs.privkey2wif(key.privkey);
      }
  ...
  floCrypto.translateAddress = function(address) {
    try {
      ...
      if (sourceChain === 'BTC') {
        ...
        if (!decodedBytes || decodedBytes.length < 25) throw new Error("Invalid address");
      ...
      const dogeChecksum = Crypto.SHA256(Crypto.SHA256(dogeBytes, {asBytes: true}), {asBytes: true}).slice(0, 4);
      ...
      const floChecksum = Crypto.SHA256(Crypto.SHA256(floBytes, {asBytes: true}), {asBytes: true}).slice(0, 4);
      result.FLO = Bitcoin.Base58.encode(floBytes.concat(floChecksum));

      // Generate address for BTC
      try {
        const words = coinjs.bech32_convert(Crypto.util.hexToBytes(hash160), 8, 5, true);
